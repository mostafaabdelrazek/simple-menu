...
export default function AdminMenus({ menus }) {
    function search(e) {
        e.preventDefault();
        const value = new FormData(e.target).get('search');

        router.get('/admin/menus', { search: value || undefined }, { preserveState: true, replace: true });
    }
            ...
            <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white">
                <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-stone-100">
                        {menus.data.map((menu) => (
                            <tr key={menu.id}>
                                <td className="px-4 py-3 font-medium text-stone-900">{menu.name}</td>
                                <td className="px-4 py-3 text-stone-600">{menu.restaurant}</td>
                                <td className="px-4 py-3 text-stone-600">{menu.currency}</td>
                                <td className="px-4 py-3 text-stone-600">{menu.languages.join(', ')}</td>
                                <td className="px-4 py-3">
                                    <a href={menu.url} target="_blank" rel="noopener noreferrer" className="font-medium text-amber-600 hover:text-amber-700">
                                        View →
                                    </a>
                                </td>
                            </tr>
